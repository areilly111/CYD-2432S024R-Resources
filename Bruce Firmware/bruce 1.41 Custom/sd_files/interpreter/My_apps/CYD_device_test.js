...
function testGPIO() {
  var pin = 0, mode = "OUTPUT", state = 0;
  while(true) {
    var opts = {};
    opts["Pin: " + pin] = "pin";
    opts["Mode: " + mode] = "mode";
    opts["Read Digital"] = "read_d";
    opts["Read Analog"] = "read_a";
    opts["Write HIGH"] = "high";
    opts["Write LOW"] = "low";
    opts["Toggle"] = "toggle";
    opts["Back"] = "";
    var c = dialogChoice(opts);
    if(c == "") break;
    if(c == "pin") {
      var p = keyboardPrompt(String(pin), 32, "GPIO Pin (0-39)");
      if(p != "") {var pn = parseInt(p); if(!isNaN(pn) && pn >= 0 && pn <= 39) pin = pn;}
    }
    else if(c == "mode") {
      var m = {"INPUT": "INPUT", "OUTPUT": "OUTPUT", "INPUT_PULLUP": "INPUT_PULLUP"};
      var mc = dialogChoice(m);
      if(mc != "") {mode = mc; gpio.pinMode(pin, mode);}
    }
    else if(c == "read_d") {
      gpio.pinMode(pin, "INPUT");
      var v = gpio.digitalRead(pin);
      dialogMessage("Pin " + pin + "\nDigital: " + (v ? "HIGH" : "LOW"), true);
    }
    else if(c == "read_a") {
      var av = gpio.analogRead(pin);
      dialogMessage("Pin " + pin + "\nAnalog: " + av, true);
    }
    else if(c == "high") {
      ...
      dialogMessage("Pin " + pin + "\nToggled to " + (state ? "HIGH" : "LOW"), true);
    }
    fillScreen(0);
  }
}
...
function testDisplay() {
  ...
  var test_colors = [0xFF0000, 0x00FF00, 0x0000FF, 0xFFFF00, 0xFF00FF, 0x00FFFF, 0xFFFFFF];
...
function testIRTransmitter() {
  ...
  while(true) {
    ...
    if(result) {
      ...
      dialogError("Failed to send IR signal", true);
    }
...
var menu_handlers = {touch: testTouchscreen, gpio: testGPIO, buzzer: testBuzzer, display: testDisplay, rgb_led: testRGBLED, ir_test: testIRMenu, settings: settingsMenu};
