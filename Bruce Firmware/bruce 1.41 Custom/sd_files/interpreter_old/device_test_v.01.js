...
function playMarioTheme() {
  ...
  // Notes and durations (freq, duration_ms)
  var mario_notes = [
    {freq: 330, duration: 100}, {freq: 330, duration: 100}, {freq: 330, duration: 100},
    {freq: 262, duration: 100}, {freq: 330, duration: 100}, {freq: 392, duration: 200},
    {freq: 196, duration: 200}, {freq: 196, duration: 100}, {freq: 262, duration: 100},
    {freq: 330, duration: 100}, {freq: 392, duration: 100}, {freq: 440, duration: 100},
    {freq: 494, duration: 300}, {freq: 392, duration: 100}, {freq: 330, duration: 100},
    {freq: 330, duration: 100}, {freq: 330, duration: 100}, {freq: 330, duration: 100},
    {freq: 262, duration: 100}, {freq: 330, duration: 100}, {freq: 392, duration: 200},
    {freq: 196, duration: 200}, {freq: 196, duration: 100}, {freq: 262, duration: 100},
    {freq: 330, duration: 100}, {freq: 392, duration: 100}, {freq: 440, duration: 100},
    {freq: 494, duration: 300}
  ];

  for(var i = 0; i < mario_notes.length; i++) {
    audio.tone(mario_notes[i].freq, mario_notes[i].duration);
    delay(50);
  }
}
...
function testBuzzer() {
  ...
  buzzer_modes["Mario Bros Theme"] = "mario";
  buzzer_modes["Back"] = "";
  ...
  while(true) {
    ...
    if(choice == "short") {
      ...
      playMarioTheme();
      dialogMessage("Mario Bros Theme completed", true);
    }
...
function testRGBLED() {
  // Initialize RGB LED pins as outputs for PWM
  var pins = [gpio_config.rgb_red_pin, gpio_config.rgb_green_pin, gpio_config.rgb_blue_pin];
...
function viewOSTasks() {
  var os_info = "System Information:\n\n";

  try {
    var heap_info = device.getFreeHeapSize();
    os_info += "Device: " + device.getName() + "\n";
    os_info += "Board: " + device.getBoard() + "\n";
    os_info += "RAM Free: " + Math.floor(heap_info.ram_free / 1024) + " KB\n";
    os_info += "Battery: " + device.getBatteryCharge() + "%\n";
  } catch(e) {
    os_info += "Device: ESP32\n";
    os_info += "Status: Running\n";
  }

  dialogMessage(os_info, true);
}

function taskManager() {
  while(true) {
    var task_options = {};
    task_options["Add Task"] = "add";
    task_options["View Tasks (" + tasks.length + ")"] = "view";
    task_options["View OS Tasks"] = "os_tasks";
    task_options["Clear All Tasks"] = "clear";
    task_options["Back"] = "";

    var choice = dialogChoice(task_options);
    ...
    if(choice == "add") {
      var task_name = keyboardPrompt("", 50, "Enter task name");
      if(task_name != "") {
        tasks.push(task_name);
        serialPrintln("Task added: " + task_name);
        dialogMessage("Task added!\n" + task_name, true);
      }
    }
    else if(choice == "view") {
      if(tasks.length == 0) {
        dialogMessage("No tasks yet", true);
      } else {
        var task_list = "Tasks (" + tasks.length + "):\n";
        for(var i = 0; i < tasks.length && i < 5; i++) {
          task_list += (i + 1) + ". " + tasks[i] + "\n";
        }
        if(tasks.length > 5) {
          task_list += "...and " + (tasks.length - 5) + " more";
        }
        dialogMessage(task_list, true);
      }
    }
    else if(choice == "os_tasks") {
      viewOSTasks();
    }
    else if(choice == "clear") {
      tasks = [];
      serialPrintln("All tasks cleared");
      dialogMessage("All tasks cleared!", true);
    }
...
function settingsMenu() {
  while(true) {
    ...
    settings_options["RGB Red Pin (current: " + gpio_config.rgb_red_pin + ")"] = "rgb_red_pin";
    settings_options["RGB Green Pin (current: " + gpio_config.rgb_green_pin + ")"] = "rgb_green_pin";
    settings_options["RGB Blue Pin (current: " + gpio_config.rgb_blue_pin + ")"] = "rgb_blue_pin";
    settings_options["Task Manager"] = "tasks";
    ...
    dialogMessage(pin_name + " pin set to: " + pin_num, true);
