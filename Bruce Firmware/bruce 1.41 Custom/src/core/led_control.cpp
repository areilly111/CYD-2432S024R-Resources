...
static int currentLedBrightness = 100;
static CRGB currentLedColor = CRGB::Black;

static uint8_t scaleChannel(uint8_t value, int brightness) {
    return static_cast<uint8_t>((static_cast<uint16_t>(value) * brightness) / 100);
}

static uint8_t applyInvert(uint8_t value) {
    return RGB_LED_INVERT ? static_cast<uint8_t>(255 - value) : value;
}

static void writeRgbPwm(const CRGB &color) {
    uint8_t r = scaleChannel(color.r, currentLedBrightness);
    uint8_t g = scaleChannel(color.g, currentLedBrightness);
    uint8_t b = scaleChannel(color.b, currentLedBrightness);

    ledcWrite(RGB_LED_R, applyInvert(r));
    ledcWrite(RGB_LED_G, applyInvert(g));
    ledcWrite(RGB_LED_B, applyInvert(b));
}
...
void beginLed() {
    ...
    // For discrete RGB LED (not addressable), use PWM on R, G, B pins
    // ledcAttach signature: ledcAttach(pin, freq, resolution)
    // ledcWrite signature: ledcWrite(pin, value)
    // Note: ESP32 Arduino 3.x uses ledcAttach which auto-configures 8-bit resolution
    ledcAttach(RGB_LED_R, RGB_LED_PWM_FREQ, 8);
    ledcAttach(RGB_LED_G, RGB_LED_PWM_FREQ, 8);
    ledcAttach(RGB_LED_B, RGB_LED_PWM_FREQ, 8);

    // Initialize LED to off state immediately
    uint8_t offValue = RGB_LED_INVERT ? 255 : 0;
    ledcWrite(RGB_LED_R, offValue);
    ledcWrite(RGB_LED_G, offValue);
    ledcWrite(RGB_LED_B, offValue);
    ...
    */
    ...
    currentLedColor = CRGB::Black; // Start with LED off
    // Apply configured color and brightness
...
void ledSetup() {
    if (bruceConfig.ledEffect == LED_EFFECT_SOLID) {
        ledEffects(false);                 // Stop any running effect task
        setLedColor(bruceConfig.ledColor); // Apply the solid color
    }
...
void ledPreviewMode(bool enable) {
    ...
    if (enable) {
        ...
        previewLedEffectSpeed = bruceConfig.ledEffectSpeed;
        previewLedEffectDirection = bruceConfig.ledEffectDirection;
        ledEffects(true); // Start preview effects
    } else {
        // Exiting preview - apply actual config
        ledEffects(false); // Stop preview task first
    }
}
